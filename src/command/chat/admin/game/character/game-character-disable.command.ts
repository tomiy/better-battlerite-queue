...
const data = new SlashCommandSubcommandBuilder()
    .setName(CommandName.GAME_CHARACTER_DISABLE)
    .setDescription('Disable character')
...
async function execute(
    ...
    if (updatedCharacter) {
        await tempReply(
            interaction,
            `Character ${context.gameModeCharacter.character.name} disabled!`,
        );
    }
}
...
export const disableCharacter: ChatInputSubcommand = {
    data: data,
